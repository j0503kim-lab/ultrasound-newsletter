        ...
        if r.status_code == 200:
            ...
            for rec in r.json().get("results", []):
                items.append({
                    ...
                    "snippet":  f"신청자: {rec.get('applicant','N/A')} | K번호: {rec.get('k_number','N/A')}",
...
def fetch_arxiv_papers(query="ergonomics", max_results=8) -> list:
    ...
    for entry in root.findall("atom:entry", ns):
        ...
        papers.append({"title":entry.findtext("atom:title","",ns).strip().replace("\n"," "),
...
def fetch_semantic_scholar(query="ergonomics workplace", max_results=10) -> list:
    ...
    try:
        ...
        for p in r.json().get("data",[]):
            ...
            papers.append({"title":p.get("title",""),
...
def call_gemini(prompt: str, data_json: str) -> str:
    ...
    payload = {"contents":[{"parts":[{"text":prompt.replace("{{DATA_JSON}}", data_json)}]}],
...
COMPANY_MAP = {
    ...
    "CHISON":               ["chison"],
    "Wisonic":              ["wisonic"],
    "AI 초음파 솔루션":       ["ultromics", "caption health", "ultrasight", "dia imaging",
                              "icardio", "brightheart", "sonio", "thinksono",
                              "smartalpha", "nerveblox", "ligence", "diagnoly"],
    "FDA 인허가":            ["fda 510", "510(k)", "ce mark", "de novo", "fda approval",
                              "fda clearance"],
}
...
NEWS_PROMPT = """당신은 초음파 의료기기 산업 전문 애널리스트입니다. 아래 뉴스를 회사별로 그룹핑하여 HTML 뉴스레터를 만드세요.
...
<div style="margin:20px 0 8px;padding:10px 16px;background:#1a5276;border-radius:6px;">
  <h3 style="color:#fff;margin:0;font-size:16px;">🏢 회사명 (뉴스 N건)</h3>
</div>
...
<div style="background:#f8f9fa;border-left:4px solid #2e86c1;padding:12px 16px;margin:4px 0 4px 16px;">
  ...
  <h4 style="margin:4px 0;font-size:14px;"><a href="URL" style="color:#1a5276;text-decoration:none;">기사 제목</a></h4>
  <p style="font-size:11px;color:#777;margin:2px 0;">📅 날짜 · 📰 출처</p>
...
PAPERS_PROMPT = """당신은 인간공학 분야 전문 연구자입니다. 아래 논문 데이터로 HTML 뉴스레터를 만드세요.

## 번역 품질 기준 (핵심)
- 한국어는 반드시 자연스러운 문장으로 작성 (기계 번역 금지)
- 전문 연구자가 쓴 것처럼 매끄럽게
- 숫자·고유명사·저자명은 영어 유지
...
<div style="background:#f8f9fa;border-left:4px solid #27ae60;padding:14px 16px;margin:12px 0;">
  ...
  <p style="font-size:12px;color:#777;margin:4px 0 8px;line-height:1.8;">👤 저자<br/>🏛️ 소속기관<br/>📖 저널 · 📅 날짜 [DOI있으면: · <a href="https://doi.org/DOI" style="color:#2e86c1;">원문 보기</a>]</p>
...
주제: 🦴 근골격계질환 / 🏥 임상 인간공학 / 🔧 작업환경 개선 / 📐 생체역학 / 📊 역학·통계 / 🧠 AI·기술 / 🩺 초음파 인간공학 / 🔬 기타
...
def build_news_html(items: list) -> str:
    """회사별 그룹핑 — Gemini 성공 시 AI 버전, 실패 시 fallback"""
    if not items:
    ...
    if GEMINI_API_KEY:
        all_html = ""
        chunk_size = 25
        for i in range(0, min(len(items), 75), chunk_size):
...
def build_papers_html(papers: list) -> str:
    """논문 섹션 — Gemini 성공 시 AI 버전, 실패 시 fallback"""
    if not papers:
    ...
    for p in papers[:10]:
        ...
        html += (
            ...
            f'<p style="font-size:11px;color:#27ae60;font-weight:bold;margin:0 0 4px;">{topic}</p>'
...
<table width="700" cellpadding="0" cellspacing="0"
  ...
  <tr><td style="padding:20px 30px;">
    <h2 style="color:#1a5276;border-bottom:2px solid #2e86c1;padding-bottom:8px;">
      📰 초음파 산업 동향</h2>
    {news_html}
  </td></tr>
  ...
  <tr><td style="padding:20px 30px;">
    <h2 style="color:#1a5276;border-bottom:2px solid #27ae60;padding-bottom:8px;">
      📄 인간공학 논문 동향</h2>
    {papers_html}
  </td></tr>
