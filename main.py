...
def _single_line_summary(text: str, max_len: int = 140) -> str:
    text = normalize_space(text)
    text = text.replace("\n", " ")
    if not text:
        return ""
    parts = re.split(r"(?<=[.!?])\s+", text)
    first = normalize_space(parts[0]) if parts else text
...
def news_summary_bilingual(item: dict) -> dict:
    ...
    prompt = f"""
다음 뉴스 1건을 사실 범위를 벗어나지 않게 한 줄로 요약하세요.
반드시 JSON만 출력합니다.
{{
  "en_summary": "...",
  "ko_summary": "..."
...
규칙:
- en_summary: 영어 한 문장만 작성
- ko_summary: 한국어 한 문장만 작성
- 각 문장은 기사에서 실제로 확인되는 최신 업데이트 핵심만 담습니다.
- 회사명 또는 제품/사업 변화 내용이 드러나야 합니다.
- 과장 금지, 추측 금지, 기사에 없는 정보 추가 금지
- 줄바꿈 금지
...
"""
    ...
    if raw:
        try:
            parsed = json.loads(raw)
            en_summary = _single_line_summary(parsed.get("en_summary", ""), 150)
            ko_summary = _single_line_summary(parsed.get("ko_summary", ""), 160)
            if en_summary and ko_summary:
                return {"en_summary": en_summary, "ko_summary": ko_summary}
        except Exception:
            pass

    base_en = _single_line_summary(snippet or title, 150)
    base_ko = _single_line_summary(translate_ko(snippet or title), 160)
    return {"en_summary": base_en, "ko_summary": base_ko}
...
def build_news_html(items: list) -> str:
  ...
  <p style="font-size:12px;color:#333;line-height:1.6;margin:8px 0 6px;"><strong>한 줄 요약:</strong> {safe_html(summary["en_summary"])}</p>
  <p style="font-size:12px;color:#444;background:#eef6fb;padding:8px;border-radius:4px;line-height:1.6;margin:0;"><strong>한글 요약:</strong> {safe_html(summary["ko_summary"])}</p>
</div>'''
            )
    return "\n".join(blocks)
